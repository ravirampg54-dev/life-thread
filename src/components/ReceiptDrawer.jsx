 ...
 */
export default function ReceiptDrawer({ receipt, onClose, onSelect, connections, receiptsById, connectedReceipts }) {
  ...
  useEffect(() => {
    ...
    const unbind = bindFocusTrap(containerRef.current, onClose);
    return () => {
      unbind();
  ...
  const related = (connectedReceipts || getConnectedReceipts(receipt.id, connections, receiptsById)).slice(0, 6);
  const topReason = related[0];

  return (
    <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true" aria-labelledby="receipt-drawer-title" ref={containerRef}>
      ...
      <div className="relative h-full w-full max-w-md overflow-y-auto border-l border-white/10 bg-slate-950 animate-fadein">
        ...
        <div className="p-5">
          <h2 id="receipt-drawer-title" className="font-serif text-2xl leading-tight text-white">{receipt.title}</h2>
          ...
          <dl className="mt-4 space-y-3 border-t border-dashed border-white/10 pt-3 font-mono text-sm">
            ...
            {receipt.location && <div className="flex justify-between gap-4"><dt className="text-slate-400">Location</dt><dd className="text-slate-200 text-right">{receipt.location}</dd></div>}
          </dl>
